...
- `devport init` is now adopt-aware: when run in a directory with hardcoded ports, it scans, groups by port, proposes names, detects registry collisions and re-allocates, prints the plan, prompts for confirmation, then rewrites safe patterns (`localhost:NNNN`, `--port NNNN`) and adds the ports to the registry. Flags: `--yes` (skip prompt, good for AI agents), `--no-adopt` (force bare init), `--dry-run` (plan only, no changes).
